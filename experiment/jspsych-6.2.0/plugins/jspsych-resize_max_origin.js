...
*
*
* Max L change Apr 21 2020: if scale_factor is less than 1, set transform origin to top left to keep canvas flush against top left
**/

jsPsych.plugins["resize_max_origin"] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'resize_max_origin',
    description: '',
    parameters: {
      item_height: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var html ='<div id="jspsych-resize_max_origin-div" style="border: 2px solid steelblue; height: '+start_div_height+'px; width:'+start_div_width+'px; margin: 7px auto; background-color: lightsteelblue; position: relative">';
    html += '<div id="jspsych-resize_max_origin-handle" style="cursor: nwse-resize_max_origin; background-color: steelblue; width: 10px; height: 10px; border: 2px solid lightsteelblue; position: absolute; bottom: 0; right: 0;"></div>';
    html += '</div>';
    if (trial.prompt !== null){
      html += trial.prompt;
    }
    html += '<a class="jspsych-btn" id="jspsych-resize_max_origin-btn">'+trial.button_label+'</a>';

    // render
    display_element.innerHTML = html;

    // listens for the click
    document.getElementById("jspsych-resize_max_origin-btn").addEventListener('click', function() {
      scale();
      end_trial();
    });
    ...
    display_element.querySelector('#jspsych-resize_max_origin-handle').addEventListener('mousedown', mousedownevent);
    ...
    var scale_div = display_element.querySelector('#jspsych-resize_max_origin-div');
    ...
    function scale() {
      ...
      // here this makes sure that the canvas is still flush to top left of the screen
      // in original resize plugin, scaling made the canvas move
      if (scale_factor < 1) {
        document.getElementById("jspsych-content").style.transformOrigin = "0 0";
      } else if (scale_factor > 1) {
        document.getElementById("jspsych-content").style.transformOrigin = "50% 50%";
      }
